...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.execute("CREATE SEQUENCE visionlouisville_user_groups_id_seq")
        db.execute("SELECT setval('visionlouisville_user_groups_id_seq', (SELECT MAX(id) FROM visionlouisville_user_groups))")
        db.execute("ALTER TABLE visionlouisville_user_groups "
            "ALTER COLUMN id SET DEFAULT nextval('visionlouisville_user_groups_id_seq'::regclass), "
            "ALTER COLUMN id SET NOT NULL")
        # db.execute("ALTER SEQUENCE visionlouisville_user_groups_id_seq OWNED BY visionlouisville_user_groups_id.seq")

        db.execute("CREATE SEQUENCE visionlouisville_user_user_permissions_id_seq")
        db.execute("SELECT setval('visionlouisville_user_user_permissions_id_seq', (SELECT MAX(id) FROM visionlouisville_user_user_permissions))")
        db.execute("ALTER TABLE visionlouisville_user_user_permissions "
            "ALTER COLUMN id SET DEFAULT nextval('visionlouisville_user_user_permissions_id_seq'::regclass), "
            "ALTER COLUMN id SET NOT NULL")
        # db.execute("ALTER SEQUENCE visionlouisville_user_user_permissions_id_seq OWNED BY visionlouisville_user_user_permissions_id.seq")

    def backwards(self, orm):
        db.execute("ALTER TABLE visionlouisville_user_groups "
            "ALTER COLUMN id DROP NOT NULL, "
            "ALTER COLUMN id DROP DEFAULT")
        db.execute("DROP SEQUENCE visionlouisville_user_groups_id_seq")

        db.execute("ALTER TABLE visionlouisville_user_user_permissions "
            "ALTER COLUMN id DROP NOT NULL, "
            "ALTER COLUMN id DROP DEFAULT")
        db.execute("DROP SEQUENCE visionlouisville_user_user_permissions_id_seq")
    ...
    models = {
        ...
        u'visionlouisville.vision': {
            ...
            'app_tweet': ('django.db.models.fields.related.OneToOneField', [], {'blank': 'True', 'related_name': "'app_tweeted_vision'", 'unique': 'True', 'null': 'True', 'to': u"orm['visionlouisville.Tweet']"}),
            ...
            'tweet': ('django.db.models.fields.related.OneToOneField', [], {'related_name': "'user_tweeted_vision'", 'unique': 'True', 'null': 'True', 'to': u"orm['visionlouisville.Tweet']"}),
            'tweeted_at': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now', 'blank': 'True'}),
