...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for vision in orm.Vision.objects.all():
            if vision.tweet_id:
                tweet = vision.tweet.tweet_data
            elif vision.app_tweet_id:
                tweet = vision.app_tweet.tweet_data
            else:
                continue

            if 'created_at' in tweet:
                vision.tweeted_at = datetime.strptime(tweet['created_at'], '%a %b %d %H:%M:%S +0000 %Y')
                vision.tweeted_at = vision.tweeted_at.replace(tzinfo=utc)
                vision.save()

        for reply in orm.Reply.objects.all():
            if reply.tweet_id:
                tweet = reply.tweet.tweet_data
            else:
                continue

            if 'created_at' in tweet:
                reply.tweeted_at = datetime.strptime(tweet['created_at'], '%a %b %d %H:%M:%S +0000 %Y')
                reply.tweeted_at = reply.tweeted_at.replace(tzinfo=utc)
                reply.save()
