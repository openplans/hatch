...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        n = 0
        for user_groups in orm.User.groups.through.objects.all():
            n += 1
            # We can't save through the ORM because the items don't yet have ids!
            db.execute(
                "UPDATE visionlouisville_user_groups "
                "   SET id=%s "

                " WHERE user_id=%s "
                "   AND group_id=%s" %
                (n, user_groups.user_id, user_groups.group_id))

        n = 0
        for user_user_permissions in orm.User.user_permissions.through.objects.all():
            n += 1
            db.execute(
                "UPDATE visionlouisville_user_user_permissions "
                "   SET id=%s "

                " WHERE user_id=%s "
                "   AND user_permission_id=%s" %
                (n, user_user_permissions.user_id, user_user_permissions.user_permission_id))
